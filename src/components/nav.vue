<template>
<nav class="nav c-page" ref="scrollWrap" :class="{'active': isMob && this.$store.state.menuShow}">
        <vue-scroll :ops="ops">
          <div class="dl" v-for="(item, index) in navList" :key="index">
            ...
            <el-collapse-transition>
              <div class="child-wrap" v-if="item.list" v-show="activeIndex === index">
                  <div class="child-gap">
                    <div v-resetClick="3000" @click="navActiveFn(childItem,j,$event)" class="dd" :class="{'active': $route.path.indexOf(childItem.route) > -1}" v-for="(childItem, j) in item.list" :key="j" >{{childItem.name}}</div>
                  </div>
              </div>
            </el-collapse-transition>
          </div>
