<template>
  <div class="c-page media-wrap mar-right">
    ...
    <problem-dialog :tabId="tabId" ref="dialog" @okDialog="okDialog" :isShow="isShow" :idArr="idArr" :refStateId="rowStatus"></problem-dialog>
  </div>
</template>
				...
				if (this.total !== 0) {
					...
					this.$fetch.post(`/system/feedBack/queryProblemFeedBack`, {
						...
						if (res.code === 0) {
							...
							if (this.list.length > 0 && this.userSourceArr.length === 0) {
                setTimeout(() => {
                	this.getType()
                }, 1000)
              }
