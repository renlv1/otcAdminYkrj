        ...
        <div class="web-table">
          <div class="center">
            <div class="common-thead">
              ...
              <div class="thead">累计业绩值</div>
              <div class="thead">累计前业绩</div>
              <div class="thead">累计后业绩</div>
            ...
            <div v-for="(item, index) in list" :key="index" class="tbody-wrap">
              <div class=" common-tbody">
                ...
                <div class="tbody">{{item.beforeTotalAmount}}</div>
                <div class="tbody">{{item.afterTotalAmount}}</div>
                <div class="tbody">{{$totalTyep(item.totalType)}}</div>
              ...
              <div class="add-more" v-show="item.packFlag === true">
                <div class="add-up">
                  <span>累计时下级业绩明细: {{item.totalDetail}}</span>
                  <span>累计时的计算明细: {{item.totalStr}}</span>
                  <span>累计折扣券数量: {{item.totalDiscount}}</span>
                  <span>累计前折扣券数量: {{item.beforeDiscount}}</span>
                  <span>累计后折扣券数量:{{item.afterDiscount}}</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      getType() {
        this.$fetch.get(`/public/enumValue?classPackage=trade.BalanceTotalLog$TotalTypeEnum&flag=true&state=1`).then(res => {
          if (res.code === 0) {
      ...
      getData(pageIndex,pageSize) {
        ...
        this.loadShow = true
        this.$fetch.post('/trade/balanceTotalLog/queryBalanceTotalLogList', {
          id: this.userId,
          ...
          createAtStr:  this.$changeDate(this.startTime, '-', 8),
          updateAtStr:  this.$changeDate(this.endTime, '-', 8),
