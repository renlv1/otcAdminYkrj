    ...
    <div class="page-wrap detail-cont">
      <ul class="ul-w ul-input-w">
        ...
        <li class="li-item li-item-text">
          ...
          <div class="li-right">{{userId}}</div>
        </li>
        <li class="li-item li-item-text">
          <div class="li-left">用户地址</div>
          <div class="li-right">{{userAddress}}</div>
        </li>
        <li class="li-item">
...
<script>
  ...
  export default {
    ...
    methods: {
      getStatus() {
        this.$fetch.get(`/public/enumValue?classPackage=finance.BankInfo$BankTypeEnum;finance.BankInfo$BankStatusEnum;finance.BankInfo$SupportDepositEnum &flag=true&state=1`).then(res => {
          if (res.code === 0) {
      ...
      paySure() {
	      ...
	      this.$fetch.post('/finance/bank/updateBankInfo', obj).then(res => {
		      if (res.code === 0) {
		        this.showDialog = false
			      Dialog({
				      content: '保存成功',
				      okFn: () => {
					      this.$router.push('/trade/bankInfo')
				      }
			      })
		      }
	      })
      ...
      getData() {
        this.$fetch.post(`/finance/bank/getBankDetail`,{
        	id: this.id
        }).then(res => {
          if(res.code === 0) {
            ...
            this.userAddress = this.isEmptyText(obj.userAddress)
