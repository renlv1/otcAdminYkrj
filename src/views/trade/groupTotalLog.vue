  ...
  <div class="c-page mar-right transaction-list">
    ...
    <div class="page-wrap">
      <div class="search">
        ...
        <div class="search-box">
          <div class="label">来源订单id：</div>
          <div class="input-wrap"><input v-model="sourceId" type="number"></div>
        </div>
        <div class="search-box">
          <div class="label">累计级别：</div>
          <div class="input-wrap"><input v-model="totalLevel" type="number"></div>
        </div>
        <div class="search-box">
          <div class="label">被累计用户名：</div>
          <div class="input-wrap"><input v-model.trim="userName" type="text"></div>
        </div>
        <div class="search-box">
          <div class="label">被累计用户地址：</div>
          <div class="input-wrap"><input v-model.trim="userAddress" type="text"></div>
        </div>
        <div class="search-box">
          <div class="label">来源用户名：</div>
          <div class="input-wrap"><input v-model.trim="sourceUserName" type="text"></div>
        </div>
        <div class="search-box">
          <div class="label">来源用户地址：</div>
          <div class="input-wrap"><input v-model.trim="sourceAddress" type="text"></div>
        </div>
        <div class="search-box">
        ...
        <div class="web-table">
          <div class="center">
            <div class="common-thead">
              ...
              <div class="thead">被累计用户名</div>
              <div class="thead tbody-address">被累计用户地址</div>
              <div class="thead " >来源用户名</div>
              <div class="thead tbody-address"  >来源用户地址</div>
              <div class="thead">来源订单id</div>
              <div class="thead">累计级别</div>
              <div class="thead">累计类型</div>
              <div class="thead">累计业绩</div>
            ...
            <div v-for="(item, index) in isListNull(list)" :key="index" class="tbody-wrap">
              <div class=" common-tbody">
                ...
                <div class="tbody " >{{item.sourceUserName}}</div>
                <div class="tbody tbody-address" >{{item.sourceAddress}}</div>
                <div class="tbody">{{item.sourceId}}</div>
                <div class="tbody">{{item.totalLevel}}</div>
                <div class="tbody">{{$sumType(item.totalType)}}</div>
                <div class="tbody">{{item.totalAmount}}</div>
              ...
              <div class="add-more" v-show="item.packFlag === true">
                <div class="add-up">
                  <span>达尔文累计金额: {{item.otherAmount}}</span> <span>累计完成后剩余的计算值: {{item.computeAmount}}</span> <span>累计增加的优惠值: {{item.discountAmount}}</span> <span>已经到账的优惠值: {{item.accountDiscountAmount}}</span> <span>创建时间:{{item.createAt}}</span> <span>更新时间:{{item.updateAt}}</span>
                </div>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getType() {
        this.$fetch.get(`/public/enumValue?classPackage=trade.GroupTotalLog$TotalTypeEnum&flag=true&state=1`).then(res => {
          if (res.code === 0) {
            this.typeArr = res.data.data.TotalTypeEnum
          }
        })
      },
      ...
      getData(pageIndex,pageSize) {
        ...
        this.loadShow = true
        this.$fetch.post('/trade/groupTotalLog/queryGroupTotalLogList', {
          id: this.userId,
          userName: this.userName, //
          ...
          totalType:this.statusType === 'selected' ? this.statusType = '' : this.statusType,
