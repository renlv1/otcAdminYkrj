        ...
        <div class="wrap">
            ...
            <div class="main-content">
                <div class="search">
                    ...
                    <div class="search-box">
                        <div class="label">额外分红所属活动id</div>
                        <div class="input-wrap"><input v-model="bonusRoundId" type="number"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">可用状态</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="handleCommand">
                                ...
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="item" v-for="(item,index) in bonusStateEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    <div class="search-box">
                        ...
                        <div class="input-wrap"><input v-model="refName" type="text"></div>
                    </div>
                    <div class="search-box">
                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                ...
                                <div class="li">可用状态</div>
                                <div class="li">参赛记录销售额</div>
                                <div class="li">额外分红金额</div>
                                <div class="li">额外分红比例</div>
                                <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                    ...
                                    <div class="li">
                                        <span v-if="item.bonusState === 0">待使用</span>
                                        <span v-else-if="item.bonusState === 1">已使用</span>
                                    </div>
                                    <div class="li">{{item.tradeTotal}}</div>
                                    <div class="li">{{item.bonusAmount}}</div>
                                    <div class="li">{{item.bonusPre}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            <div class="li">额外分红消耗业绩: {{item.useTotal}}</div>
                                            <div class="li">相关币种: {{item.totalCurrency}}</div>
                                            <div class="li">额外分红所属业绩积累: {{item.bonusGroupId}}</div>
                                            <div class="li">额外分红所属参赛记录: {{item.bonusPlayerId}}</div>
                                            <div class="li">额外分红所属活动id: {{item.bonusRoundId}}</div>
                                            <div class="li">分红时间: {{item.bonusAt}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getType () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.LottoBonusRecord$BonusStateEnum;&flag=false&state=1`).then(res => {
          if (res.msg === '成功') {
            let data = res.data.data
            this.bonusStateEnum = data.BonusStateEnum
          }
        })
      },
      ...
      getData (pageIndex,pageSize) {
        ...
        objData.bonusState = this.statusId === 'selected' ? '' : this.statusId
        if (this.refAddress) objData.refAddress = this.refAddress
        if (this.bonusRoundId) objData.bonusRoundId = this.bonusRoundId
        ...
        this.$fetch.post(`/klein/lottoBonusRecord/queryLottoBonusRecordList`,objData).then(res => {
          this.loadShow = false
          if (res.msg === '成功') {
