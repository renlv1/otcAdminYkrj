        ...
        <div class="wrap">
            ...
            <div class="main-content">
                <div class="search">
                    ...
                    <div class="search-box">
                        <div class="label">变化关联id</div>
                        <div class="input-wrap"><input v-model="relateId" type="number"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">变化所属活动id</div>
                        <div class="input-wrap"><input v-model="roundId" type="number"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">帐变类型</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="handleCommand">
                                ...
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="item" v-for="(item,index) in changeTypeEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                ...
                                <div class="li">账变类型</div>
                                <div class="li">变化金额</div>
                                <div class="li">变化前金额</div>
                                <div class="li">变化后金额</div>
                                <div class="li">变化关联ID</div>
                                <div class="li">活动所属活动ID</div>
                                <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                    ...
                                    <div class="li">{{matchType(changeTypeEnum,item.changeType)}}</div>
                                    <div class="li">{{item.changeAmount}}</div>
                                    <div class="li">{{item.balanceBefore}}</div>
                                    <div class="li">{{item.balanceFinal}}</div>
                                    <div class="li">{{item.relateId}}</div>
                                    <div class="li">{{item.roundId}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      handleCommand (commend) {
       this.changeTypeText = commend.text
       this.changeTypeId = commend.id
      },
      unfold (index) {
      ...
      getType () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.LottoBonusLog$ChangeTypeEnum;&flag=true&state=1`).then(res => {
          if (res.msg === '成功') {
            let data = res.data.data
            this.changeTypeEnum = data.ChangeTypeEnum
          }
        })
      },
      search () {
      ...
      getData (pageIndex,pageSize) {
        this.loadShow = true
        /*let changeType = parseInt(this.changeTypeId, 10) >= 0 ? parseInt(this.changeTypeId) : ''*/
        let objData ={
        ...
        if (this.relateId) objData.relateId = this.relateId
        if (this.roundId) objData.roundId = this.roundId
        objData.changeType = this.changeTypeId === 'selected' ? '' : this.changeTypeId
        ...
        if (this.total !== 0) {
          this.$fetch.post(`/klein/lottoBonusLog/queryLottoBonusLogList`,objData).then(res => {
            this.loadShow = false
            if (res.msg === '成功') {
