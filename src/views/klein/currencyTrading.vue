                ...
                <div class="search">
                    ...
                    <div class="search-box">
                        ...
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="stateCommand">
                        ...
                        <div class="center">
                            ...
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                    ...
                                    <div class="li">{{stateFn(item.flag)}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            ...
                                            <div class="li"><span>成交手续费: </span>{{item.tradeFee}}</div>
                                            <div class="li">创建时间: {{item.createAt}}</div>
                                            <div class="li">更新时间: {{item.updateAt}}</div>
                                            <div class="li"><span>成交时间: </span>{{item.finishAt}}</div>
                                            <div class="li">买卖标识: {{flagFn(item.flag)}}</div>
                                            <div class="li">撮合最处理标识: {{processFlagFn(item.processFlag)}}</div>
                                            <div class="li"><span>买家用户地址: </span>{{item.buyAddress}}</div>
                                            <div class="li"><span>卖家用户地址: </span>{{item.sellAddress}}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getType () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.NewCointradeRecord$StateEnum&flag=true&state=1`).then(res => {
          if (res.msg === '成功') {
            this.stateEnum = res.data.data.StateEnum
          }
        })
      },
      stateCommand (commend) {
         this.stateText = commend.text
         this.stateId = commend.id
      },
      ...
      getData (pageIndex,pageSize) {
        this.loadShow = true
        let stateId = parseInt(this.stateId, 10) >= 0 ? parseInt(this.stateId, 10) : ''
        let objData ={
        ...
        if (stateId) objData.state = stateId
        ...
        if (this.total !== 0) {
          this.$fetch.post(`/klein/newCointrade/queryNewCointradeList`,objData).then(res => {
            this.loadShow = false
            if (res.msg === '成功') {
