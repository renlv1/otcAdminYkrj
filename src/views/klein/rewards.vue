                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                ...
                                <div class="li">开通ai订单编号</div>
                                <div class="li">参与奖励标识</div>
                                <div class="li">上级奖励状态</div>
                                <div class="li">循环奖励状态</div>
                                <div class="li">累计奖励金额</div>
                                <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                    ...
                                    <div class="li">{{item.orderId}}</div>
                                    <div class="li">{{item.playerIndex}}</div>
                                    <div class="li">{{matchType(refStateEnum,item.refRewardState)}}</div>
                                    <div class="li">{{matchType(roundRewardStateEnum,item.roundRewardState)}}</div>
                                    <div class="li">{{item.rewardSum}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            <div class="li">活动Id: {{item.roundId}}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getType () {
        let url = '/public/enumValue?classPackage=klein.DanRewardPlayer$RefStateEnum;klein.DanRewardPlayer$RoundRewardStateEnum&flag=true&state=1'
        this.$fetch.post(`${url}`).then((res) => {
          if (res.msg === '成功') {
            this.refStateEnum = res.data.data.RefStateEnum
            this.roundRewardStateEnum = res.data.data.RoundRewardStateEnum
          }
        })
      },
      search () {
      ...
      getData (pageIndex,pageSize) {
        ...
        if (this.total !== 0) { 
          this.$fetch.post(`/klein/danRewardPlayer/queryDanRewardPlayer`,{
            id: this.id,
            ...
            refRewardState: refStateId,
            roundRewardState: roundRewardId,
