        ...
        <div class="wrap">
            ...
            <div class="main-content">
                <div class="search">
                    ...
                    <div class="search-box">
                        <div class="label">被奖励用户名</div>
                        <div class="input-wrap"><input v-model.trim="userName" type="text"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">被奖励推荐人用户名</div>
                        <div class="input-wrap"><input v-model.trim="refName" type="text"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">奖励来源活动id</div>
                        <div class="input-wrap"><input v-model="rewardRoundId" type="number"></div>
                    </div>
                    <div class="search-box">
                    ...
                    <div class="search-box">
                        <div class="label">奖励发放状态</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="commandChange">
                                <span class="el-dropdown-link">
                                  {{rewardStateText  || '全部'}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu class="menu" slot="dropdown">
                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                 <div class="li">ID</div>
                                 <div class="li">被奖励用户名</div>
                                 <div class="li">被奖励用户地址</div>
                                 <div class="li"><span>被奖励推荐人用户名</span></div>
                                 <div class="li">被奖励推荐人地址</div>
                                 <div class="li">奖励类型</div>
                                 <div class="li">奖励发放状态</div>
                                 <div class="li">投资金额</div>
                                 <div class="li">实际奖励金额</div>
                                 <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                     ...
                                     <div class="li">{{item.joinAmount}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            <div class="li">规划奖励金额: {{item.ruleAmount}}</div>
                                            <div class="li">奖励币种: {{item.rewardCurrency}}</div>
                                            <div class="li">奖励来源活动id:{{item.rewardRoundId}}</div>
                                            <div class="li">奖励来源分红任务: {{item.rewardJobId}}</div>
                                            <div class="li">奖励所属参与id: {{item.yourPlayerId}}</div>
                                            <div class="li">奖励支付id: {{item.rewardPayId}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      rewardTypeFn (length) {
        let text = ''
        this.rewardTypeEnum.forEach(item => {
      ...
      rewardStateFn (length) {
        let text = ''
        this.rewardStateEnum.forEach(item => {
          if (parseInt(item.id) === parseInt(length)){
            text = item.text
          }
        })
      ...
      commandChange (commend) {
        if (commend.index === 1) {
          ...
          this.rewardStateText = commend.item.text
          this.rewardStateId = commend.item.id
        }
      },
      typeData () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.LottoRewardRecord$RewardTypeEnum;klein.LottoRewardRecord$RewardStateEnum&flag=true&state=1`).then(res => {
          if (res.msg === '成功') {
            let data = res.data.data
            this.rewardTypeEnum = data.RewardTypeEnum
      ...
      getData (pageIndex,pageSize) {
        ...
        if (this.rewardRoundId) objData.rewardRoundId = this.rewardRoundId
        objData.rewardType = this.rewardId === 'selected' ? '' : this.rewardId
        objData.rewardState = this.rewardStateId === 'selected' ? '' : this.rewardStateId
        ...
        if (this.total !== 0) {
           this.$fetch.post(`/klein/lottoRewardRecord/queryLottoRewardRecordList`,objData).then(res => {
              this.loadShow = false
              if (res.msg === '成功') {
