        ...
        <div class="wrap">
            ...
            <div class="main-content">
                <div class="search">
                    ...
                    <div class="search-box">
                        <div class="label">推荐人用户地址</div>
                        <div class="input-wrap"><input v-model.trim="refAddress" type="text"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">数据同步状态</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="commandChange">
                                <span class="el-dropdown-link">
                                  {{synchStateText || '全部'}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="composeValue(item, 1)" v-for="(item,index) in synchStateEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    <div class="search-box">
                        <div class="label">是否大派别</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="commandChange">
                                <span class="el-dropdown-link">
                                  {{isBigText || '全部'}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="composeValue(item, 2)" v-for="(item,index) in isBigEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                 ...
                                 <div class="li">数据同步状态</div>
                                 <div class="li">是否大派别</div>
                                 <div class="li">已消耗业绩</div>
                                 <div class="li">派别业绩</div>
                                 <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                     ...
                                     <div class="li">{{matchType(synchStateEnum,item.synchState)}}</div>
                                     <div class="li">{{matchType(isBigEnum,item.isBig)}}</div>
                                     <div class="li">{{item.usedTotal}}</div>
                                     <div class="li">{{item.groupTotal}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            <div class="li">剩余可用业绩: {{item.usableTotal}}</div>
                                            <div class="li">个人业绩: {{item.personTotal}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getType () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.LottoGroupTotal$SynchStateEnum;klein.LottoGroupTotal$IsBigEnum;&flag=true&state=1`).then(res => {
           if (res.msg === '成功') {
             let data = res.data.data
             this.isBigEnum = data.IsBigEnum
             this.synchStateEnum = data.SynchStateEnum
           }
        })
      },
      commandChange (commend) {
        if (commend.index === 1) {
          this.synchStateText = commend.item.text
          this.synchStateId = commend.item.id
        } else if (commend.index === 2) {
          this.isBigText = commend.item.text
          this.isBigId = commend.item.id
        }
      },
      search () {
        this.total = 1
      ...
      getData (pageIndex,pageSize) {
        ...
        objData.isBig = this.isBigId === 'selected' ? '' : this.isBigId
        objData.synchState = this.synchStateId === 'selected' ? '' : this.synchStateId
        ...
        this.$fetch.post(`/klein/lottoGroupTotal/queryLottoGroupTotalList`,objData).then(res => {
           if (res.msg === '成功') {
             this.loadShow = false
