            ...
            <div class="main-content">
                <div class="search">
                    ...
                    <div class="search-box">
                        ...
                        <div class="input-wrap"><input v-model="roundId" type="number" onKeypress="return(/[\d\.]/.test(String.fromCharCode(event.keyCode)))"></div>
                    </div>
                    <div class="search-box">
                        <div class="label">打赏状态</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="commandChange">
                                <span class="el-dropdown-link">
                                  {{awardText || '全部'}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="composeValue(item, 1)" v-for="(item,index) in awardStateEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    <div class="search-box">
                        <div class="label">列队状态</div>
                        <div class="input-wrap dropdown-wrap">
                            <el-dropdown @command="commandChange">
                                <span class="el-dropdown-link">
                                  {{listStateText || '请选择'}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu class="menu" slot="dropdown">
                                    <el-dropdown-item :command="composeValue(item, 2)" v-for="(item,index) in listStateEnum" :key="index">{{item.text}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                    </div>
                    ...
                    <div class="web-table">
                        <div class="center">
                            <div class="thead item-list">
                                ...
                                <div class="li"><span>推荐人用户名</span></div>
                                <div class="li">推荐人地址</div>
                                <div class="li">奖励任务状态</div>
                                <div class="li">列队状态</div>
                                <div class="li">投注金额</div>
                                <div class="li">投注币种</div>
                                <div class="li">操作</div>
                            </div>
                            <div class="item-list table-list" v-for="(item, index) in list" :key="index">
                                <div class="flex-item">
                                    ...
                                    <div class="li">{{matchType(awardStateEnum,item.awardState)}}</div>
                                    <div class="li">{{matchType(listStateEnum,item.listState)}}</div>
                                    <div class="li">{{item.playAmount}}</div>
                                    <div class="li">{{item.playCurrency}}</div>
                                ...
                                <div class="on-the-details" :class="{'active': unfoldActive === index && itemShow}">
                                    <div class="info-box" v-if="unfoldActive === index && itemShow">
                                        <div class="item-flex-item">
                                            <div class="li">亏损金额: {{item.lossAmount}}</div>
                                            <div class="li">最大打赏金额: {{item.awardAmount}}</div>
                                            <div class="li">打赏参与标识: {{item.awardIndex}}</div>
                                            <div class="li">打赏次数: {{item.awardCount}}</div>
                                            <div class="li">上次打赏时间: {{item.lastAwardAt}}</div>
                                            <div class="li">打赏累计金额: {{item.awardSum}}</div>
                                            <div class="li">打赏所属参赛记录: {{item.playerId}}</div>
                                            <div class="li">活动编号: {{item.roundId}}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getType () {
        this.$fetch.post(`/public/enumValue?classPackage=klein.LottoAwardQueue$AwardStateEnum;klein.LottoAwardQueue$ListStateEnum;&flag=true&state=1`).then(res => {
          if (res.msg === '成功') {
            let data = res.data.data
            this.awardStateEnum = data.AwardStateEnum
            this.listStateEnum = data.ListStateEnum
          }
        })
      },
      commandChange (commend) {
        if (commend.index === 1) {
          this.awardText = commend.item.text
          this.awardId = commend.item.id
        } else if (commend.index === 2) {
          this.listStateText = commend.item.text
          this.listStateId = commend.item.id
        }
      },
      search() {
        this.total = 1
      ...
      getData (pageIndex,pageSize) {
        ...
        objData.awardState = this.awardId === 'selected' ? '': this.awardId
        objData.listState = this.listStateId === 'selected' ? '': this.listStateId
        ...
        if (this.total !== 0) {
          this.$fetch.post(`/klein/lottoAwardQueue/queryLottoAwardQueueList`,objData).then(res => {
             this.loadShow = false
             if (res.msg === '成功') {
